...
export function ReferencesEditor() {
  ...
  const [draft, setDraft] = useState<ReferenceItem>(emptyReference);
  ...
  async function uploadImage(file: File) {
    ...
    setUploading(false);
    setMessage("Fotografie byla nahrána. Referenci ještě uložte.");
  }
  ...
  async function remove(item: ReferenceItem) {
    ...
    if (draft.id === item.id) setDraft(emptyReference());
    ...
    <div className="space-y-6">
      <div className="admin-panel">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <button type="button" onClick={() => setDraft(emptyReference())} className="btn btn-primary">
            Přidat referenci
          </button>
        </div>
        ...
        <div className="mt-7 space-y-3">
          ...
          {!loading && items.length === 0 && (
            <p className="rounded-xl bg-brand-50 border border-[var(--color-line)] p-5 text-brand-900/65">
              Zatím tu nejsou žádné záznamy. Po spuštění migrace se vloží tři současné reference.
            </p>
          )}
          {items.map((item) => (
            <div key={item.id} className="grid sm:grid-cols-[96px_1fr_auto] gap-4 items-center rounded-xl border border-[var(--color-line)] p-3">
              <div className="relative aspect-[4/3] rounded-lg overflow-hidden bg-brand-100">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={publicImageUrl(item.image_url)} alt="" className="absolute inset-0 w-full h-full object-cover" />
              </div>
              <div className="min-w-0">
                ...
                <p className="text-xs text-brand-900/55 mt-1">{item.category} · pořadí {item.sort_order}</p>
              </div>
              <div className="flex gap-2">
                <button type="button" onClick={() => setDraft({ ...item })} className="admin-small-button">Upravit</button>
                <button type="button" onClick={() => void remove(item)} className="admin-small-button text-red-700">Smazat</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <form onSubmit={save} className="admin-panel">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="admin-kicker">{draft.id ? "Úprava záznamu" : "Nový záznam"}</p>
            <h3 className="font-display text-3xl text-brand-900 mt-1">
              {draft.id ? draft.title : "Nová reference"}
            </h3>
          </div>
          {draft.id && (
            <button type="button" onClick={() => setDraft(emptyReference())} className="admin-small-button">
              Zavřít úpravu
            </button>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-5 mt-7">
          ...
          <EditorField label="Kategorie" value={draft.category} onChange={(value) => update("category", value)} placeholder="Např. Vzduchotechnika" />
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-brand-900">
              Popis
              <textarea value={draft.description} onChange={(e) => update("description", e.target.value)} required rows={5} className="admin-input resize-y" />
            </label>
          </div>
          <EditorField label="Alternativní popis fotografie" value={draft.alt} onChange={(value) => update("alt", value)} placeholder="Co je na fotografii" />
          <EditorField label="Pořadí" type="number" value={String(draft.sort_order)} onChange={(value) => update("sort_order", Number(value))} />
          ...
          <label className="block text-sm font-medium text-brand-900 md:col-span-2">
            ...
            <span className="block text-xs text-brand-900/45 mt-1.5">
              JPEG, PNG, WebP nebo AVIF, maximálně 6 MB. {uploading && "Nahrávám…"}
            </span>
          </label>

          <div className="md:col-span-2">
            <EditorField label="URL fotografie" value={draft.image_url} onChange={(value) => update("image_url", value)} placeholder="Vyplní se automaticky po nahrání" required={false} />
          </div>

          {draft.image_url && (
            <div className="md:col-span-2 relative aspect-[16/7] rounded-xl overflow-hidden bg-brand-100">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={publicImageUrl(draft.image_url)} alt="Náhled" className="absolute inset-0 h-full w-full object-cover" />
            </div>
          )}

          <label className="flex items-center gap-3 text-sm text-brand-900">
            <input type="checkbox" checked={draft.published} onChange={(e) => update("published", e.target.checked)} className="accent-[var(--brand-700)]" />
            Publikovat na webu
          </label>
          <label className="flex items-center gap-3 text-sm text-brand-900">
            <input type="checkbox" checked={draft.featured} onChange={(e) => update("featured", e.target.checked)} className="accent-[var(--brand-700)]" />
            Zobrazit na úvodní stránce
          </label>
          <label className="block text-sm font-medium text-brand-900">
            Velikost karty
            <select value={draft.size} onChange={(e) => update("size", e.target.value as ReferenceItem["size"])} className="admin-input">
        ...
        <div className="flex flex-wrap items-center gap-4 mt-7">
          <button type="submit" disabled={saving || uploading} className="btn btn-primary disabled:opacity-60">
...
function EditorField({
  ...
  return (
    <label className="block text-sm font-medium text-brand-900">
      {label}
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} required={required} className="admin-input" />
    </label>
  );
}

function Badge({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full bg-brand-100 px-2.5 py-1 text-[0.6rem] uppercase tracking-wider text-brand-700">{children}</span>;
}
